...
> 归档说明：本文是阶段性收敛准备稿，不再作为当前运行态入口文档。当前真实架构、Profile 解析、上下文策略和客服主链以 [../AGENT_TECHNICAL_DOCUMENTATION.md](../AGENT_TECHNICAL_DOCUMENTATION.md)、[../API_MULTI_USER_INTEGRATION.md](../API_MULTI_USER_INTEGRATION.md) 为准。
...
### 1.2 多轮历史交由 agent/checkpointer 处理

当前已取消自定义“历史上下文摘要”压缩，避免压缩后消息窗口缺少 `AIMessage` 触发 LangChain 工具调用状态异常。现行策略为：

- 保留完整消息历史，由 agent/checkpointer 处理上下文
- 仅清理历史轮次里的旧图片、语音、视频 URL
- 最新用户消息原文和当前多模态输入保持不变
...
- 上下文工程默认交给 agent/checkpointer
- 旧多媒体内容只做安全脱敏，不再压缩为摘要
...
3. 三层知识链、完整上下文交给 agent/checkpointer、多模态前处理，已经是两条链路的共同基础设施。
