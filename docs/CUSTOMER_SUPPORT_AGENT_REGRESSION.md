...
2. 前置安全拦截
3. 意图识别与 planner JSON 结构化理解
4. deterministic harness 执行链
5. 标准客服 Agent delegate 兜底
6. `smart_search` 与 `agent_browser_deep_search` 的受控知识检索
7. 船舶查询、统计、写操作、文件、多模态、网页核验链路
8. 多轮上下文压缩与相关历史筛选
9. 最终输出脱敏与链接校验
10. `/stream_run` 调试流事件
...
- `tests/test_customer_support_router.py`: `50 passed`
- `tests/test_customer_support_intent_agent.py`: `20 passed`
...
- `smart_search` 命中时，不应再触发 `agent_browser_deep_search`
- `smart_search` 无有效结果时，可触发 `agent_browser_deep_search`
- `agent-browser` 返回内容后，最终回复不能暴露内部 CLI、日志、路径
...
3. `generated_tool_calls`
   - 是否调用了预期工具
4. `route_trace.fallback_reason`
   - 是否因为 `smart_search_empty_agent_browser_fallback`、`unsupported_*` 等原因降级
5. `check_result`
   - 是否被脱敏、空答、无效链接兜底
6. 最终回复
   - 是否仍夹带搜索包装文本或内部信息
7. `latency_hotspot.total`
   - 是否存在异常慢请求

## 7. 当前已知限制

- 知识库内容不足时，`smart_search` 命中质量会直接下降
- `agent-browser` 是受控兜底，不是自由浏览器 Agent
...
1. 补知识库内容
2. 补官方网页抓取与索引覆盖
3. 提升 planner/query rewrite 的稳定性
4. 扩展异地部署联调脚本和线上观测面板
