...
| 路由 | 能力 | API 端点 | 需 Token | 方法 |
|------|------|---------|----------|------|
| A | 船舶搜索 | `/position/shipSearchText` | HIFLEET_TTSE_KEY | GET |
| B | 船位查询 | `/position/position/get/token` | HIFLEET_API_KEY | GET |
| C | 船舶档案 | `/shiparchive/getShipArchiveWithEnginAndCompany` | HIFLEET_API_KEY | GET |
| D | PSC 检查记录 | `/pscapi/get` | HIFLEET_API_KEY | GET |
| E | 区域船舶数量 | `/position/gettraffic/token` | HIFLEET_API_KEY | GET |
| F | 海峡通航统计 | `/position/statisticzonetraffic` | 可选 | POST |
| G | 船位上传 | `/position/updateShipAisInfo` | 无 | POST |
| H | 静态信息更新 | `/position/updateShipAisStaticInfo` | 无 | POST |
...
1. 调用 `scripts/search_ship.py --keyword "YU MING"`
...
2. 调用 `scripts/get_position.py --mmsi 414726000`
3. 返回实时位置、航速、航向、状态等

### Workflow 3: 船舶档案（路由 C）

1. 需要 MMSI 或 IMO
2. 调用 `scripts/get_archive.py --mmsi 414726000` 或 `--imo 9613886`
3. 返回船舶详细参数（尺寸、载重吨、建造年份等）

### Workflow 4: PSC 检查记录（路由 D）

1. 需要 IMO（若仅有 MMSI，先通过 Workflow 1/3 获取 IMO）
2. 调用 `scripts/get_psc.py --imo 9613886`
3. 返回 PSC 检查记录和滞留情况

### Workflow 5: 区域船舶查询（路由 E）

1. 用户给出区域名称或坐标范围
2. 区域名称 → 内置映射表获取 areaId（见 `config/strait_oid_map.json` 中 area_id_map）
3. 坐标范围 → 构造 bbox（`左经,下纬,右经,上纬`）或 polygon
4. 调用 `scripts/get_area_traffic.py --areaId 1`
5. 返回区域船舶数量

### Workflow 6: 海峡通航统计（路由 F）

1. 用户给出海峡名称 + 日期范围
2. 海峡名称 → OID 映射表（见 `config/strait_oid_map.json`）
3. 调用 `scripts/get_strait_traffic.py --oid 24480 --startdate 2026-05-07 --enddate 2026-05-07`
4. 返回通航统计数据（按方向、船型分类）
...
2. 经纬度格式转换（度分秒 → 十进制度），参考 `references/coord_conversion.md`
3. 需要 MMSI（若仅有船名，先执行 Workflow 1）
4. 调用 `scripts/upload_position.py --mmsi 414726000 --lon 116.875 --lat 22.125 --speed 15`
5. 返回上传结果
...
3. 调用 `scripts/update_static_info.py --mmsi 636025653 --destination LUOYUAN --eta "2026-05-09 20:00:00"`
4. 返回更新结果

## Notes

- **经纬度格式**：用户输入可能是度分秒（116°52.5′E），需转换为十进制度（116.875），详见 `references/coord_conversion.md`
