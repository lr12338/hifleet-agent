...
def get_db_url() -> str:
    ...
    if url is not None and url != "":
        return url
    # 若未配置 workload identity 所需变量，则不再抛异常，直接返回空字符串
    # 由上层决定是否强制要求Postgres checkpointer。
    required_wi_vars = [
    ...
    if not all(os.getenv(k) for k in required_wi_vars):
        logger.warning("PGDATABASE_URL and workload identity vars are not set")
        return ""
