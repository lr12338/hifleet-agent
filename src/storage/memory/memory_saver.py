...
class MemoryManager:
    ...
    def get_checkpointer(self) -> BaseCheckpointSaver:
        """获取 checkpointer，支持模式：
        - postgres: 强制Postgres，不允许回退
        - memory:   强制MemorySaver
        - auto:     优先Postgres，失败回退MemorySaver
        """
        if self._checkpointer is not None:
            return self._checkpointer

        if CHECKPOINTER_MODE == "memory":
            logger.warning("COZE_CHECKPOINTER_MODE=memory, force MemorySaver")
        ...
        if not db_url:
            if CHECKPOINTER_MODE == "postgres":
                raise RuntimeError(
                    "COZE_CHECKPOINTER_MODE=postgres but PGDATABASE_URL is unavailable. "
                    "Please set PGDATABASE_URL to enable shared persistent memory."
                )
            return self._create_fallback_checkpointer()
        ...
        if not self._setup_schema_and_tables(db_url):
            if CHECKPOINTER_MODE == "postgres":
                raise RuntimeError("COZE_CHECKPOINTER_MODE=postgres but failed to setup memory schema/tables")
            return self._create_fallback_checkpointer()
        ...
        try:
            ...
            if CHECKPOINTER_MODE == "postgres":
                raise RuntimeError(f"COZE_CHECKPOINTER_MODE=postgres but AsyncPostgresSaver init failed: {e}") from e
