...
- API 字段定义：name 必选 string 船名或 MMSI；mmsi 可选 string；lon 必选 float 经度十进制度；lat 必选 float 纬度十进制度；updatetime 必选 string，格式 yyyy-MM-dd HH:mm:ss；speed 可选 float 航速节；heading 可选 float 船首向度；course 可选 float 航迹向度；draught 可选 float 吃水米；destination 可选 string 目的港；eta 可选 string 预抵时间；status 可选 string 航行状态中文文本。
- 子 agent 必须输出工具参数：
  ...
  - speed：可选，float/string，航速（节），例如 7.1。
  - heading：可选，float/string，船首向/船艏向（度），例如 135.0。
  - course：可选，float/string，航迹向（度），例如 254.1。
  - draft：可选，float/string，吃水（米）。工具参数是 draft，工具内部映射 API draught。
  - destination：可选，string，目的港，例如 DA LIAN。
  - eta：可选，string，预抵时间，最终归一为 yyyy-MM-dd HH:mm:ss；无法归一时丢弃，不阻断船位更新。
  - navstatus：可选，string，航行状态中文文本。工具参数是 navstatus，工具内部映射 API status。
  - ship_name、wechatgroup：可选。
- 工具参数到 API 字段映射：mmsi -> name+mmsi；ship_name -> name；draft -> draught；navstatus -> status；lon/lat/speed/heading/course/updatetime/destination/eta 同名传递。
- 允许的 navstatus：
  在航 | 失控 | 帆船在航 | 搁浅 | 操纵能力受限 | 机动船在航 | 系泊 | 锚泊 | 停泊 | 未知 | 未定义 | 正在捕鱼作业 | 限于吃水 | 高速船留用 | 地效翼船留用 | 待定义
- 船位工具参数示例：
  {"mmsi":"353738000","lon":"122","lat":"31","updatetime":"2025-03-31 09:52:13","speed":"7.1","heading":"135.0","course":"254.1","draft":"4.2","destination":"DA LIAN","eta":"2025-03-27 14:00:00","navstatus":"机动船在航","ship_name":"LEO I"}

船舶静态信息更新工具：update_ship_static_info
- 输出的是工具参数，不是 API body。
- API 字段定义：mmsi 必选 string；name 可选 string 英文船名；imonumber 可选 string IMO 编号；callsign 可选 string 呼号；type 可选 string 船型描述；minotype 可选 string 船舶子类型且更新船型时同 type；width 可选 string 船宽米；length 可选 string 船长米；dwt 可选 string 载重吨；buildyear 可选 string 建造年份 yyyy；destination 可选 string 目的港；eta 可选 string 预抵时间；draught 可选 double 吃水米。
- 子 agent 必须输出工具参数：
...
- 工具参数到 API 字段映射：ship_name -> name；imo -> imonumber；ship_type -> type；built_year -> buildyear；draft -> draught；minotype/width/length/dwt/callsign/destination/eta 同名传递。
- 静态更新 ready_to_execute 必须同时有 mmsi 和至少一个非 mmsi 更新字段。
- 静态工具参数示例：
  {"mmsi":"100000278","ship_name":"CESHI","imo":"0","callsign":"0","ship_type":"散货船","minotype":"散货船","width":"12.3","length":"30.5","dwt":"12000","built_year":"2018","destination":"EE SLM","eta":"2021-05-22 00:00:00","draft":"1.0"}
...
def _coerce_tool_args(tool_name: str | None, args: dict[str, Any]) -> dict[str, Any]:
    ...
    cleaned = _normalize_optional_tool_eta(cleaned)
...
def _normalize_position_tool_args(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return _normalize_optional_tool_eta(clean_optional_voyage_fields(result))


def _normalize_optional_tool_eta(args: dict[str, Any]) -> dict[str, Any]:
    result = dict(args or {})
    if "eta" not in result:
        return result
    normalized_eta = _normalize_optional_eta_value(result.get("eta"))
    if normalized_eta:
        result["eta"] = normalized_eta
    else:
        result.pop("eta", None)
    return result


def _normalize_optional_eta_value(value: Any) -> str:
