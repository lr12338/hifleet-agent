...
8. 若当前输入/语音识别/附件识别内容包含船舶更新参数，识别并格式化输出 position_update_params 与 static_update_params
9. 输出后续执行建议，包括 route/task_type/tool_bundle/needs_harness/needs_clarification/missing_required_fields/safety_flags
...
规则：
- 这是客服链路的主决策节点。你需要深度理解当前输入后决定后续 route，不要依赖简单关键词。
...
- ship_update 包含两类：船位/动态 AIS 更新、船舶静态信息更新。
- 如果当前输入包含船舶更新参数，必须输出两个参数组：position_update_params 和 static_update_params；没有的字段填空字符串。
- 参数只可来自本轮用户文字、语音识别内容、当前附件/OCR/perception，不得用历史上下文、系统时间或推理补值。
- 船位更新参数包括：mmsi, imo, ship_name, lon, lat, updatetime, speed, heading, course, draft, navstatus, destination, eta, wechatgroup。
- 静态信息参数包括：mmsi, imo, ship_name, ship_type, minotype, length, width, dwt, flag, callsign, built_year, destination, eta, draft, wechatgroup。
- 位置/POSN 中的成对坐标按方向拆分：N/S 为 lat，E/W 为 lon，例如“17-56.73N 115-47.69E”输出 lat=17-56.73N, lon=115-47.69E。坐标保留原始格式，不转十进制度。
- “航向”默认映射 heading；明确“航迹向/COG/course”时映射 course。
- ship_type 与 minotype 是同一个“船舶类型”业务字段；用户输入“船舶类型/船型/type/minotype/MINO船型代码”时，同时填入 static_update_params.ship_type 和 static_update_params.minotype。
- 如果用户同时给出 ship_type 与 minotype 且值不同，在 low_confidence_fields 里记录冲突，且不要把冲突船型作为可直接写入依据。
- destination、eta、draft、wechatgroup 两类工具都支持，按用户主诉写入对应参数组；若用户明确同时更新船位和静态信息，两个组都可输出。
- 如果信息不足或存在安全风险，设置 needs_clarification=true，并给出 clarification_question。
- missing_required_fields 只列当前执行所需且本轮输入/附件缺失的字段，例如 ship_identifier、lon、lat、updatetime、static_update_field、mmsi_confirmation。
- safety_flags 用于标记风险，例如 ship_update_missing_identifier、ship_update_context_identifier_only、ship_name_requires_confirmation、position_update_troubleshooting_not_write、ship_type_conflict。
- 平台故障类“船位更新慢/不刷新/失败/异常/报错”不是写操作，必须避免 ship_update，并可设置 safety_flags=["position_update_troubleshooting_not_write"]。
...
JSON 格式:
{"intent":"knowledge","route":"knowledge","task_type":"platform_knowledge","tool_bundle":["local_kb_search","web_search","web_search_agent_browser"],"needs_harness":false,"confidence":"high|medium|low","reason_summary":"一句话","use_context_ship":false,"missing_slot":{"field":"","question":""},"needs_clarification":false,"clarification_question":"","missing_required_fields":[],"safety_flags":[],"rewritten_user_need":"用户真正想确认的需求描述","query_type":"hifleet_product","search_keywords":["hifleet","筛选船队","记忆功能"],"search_query_candidates":["hifleet 筛选船队 记忆功能"],"needs_multimodal_grounding":false,"should_prefer_local_kb":true,"should_limit_to_hifleet_sites":true,"position_update_params":{"mmsi":"","imo":"","ship_name":"","lon":"","lat":"","updatetime":"","speed":"","heading":"","course":"","draft":"","navstatus":"","destination":"","eta":"","wechatgroup":""},"static_update_params":{"mmsi":"","imo":"","ship_name":"","ship_type":"","minotype":"","length":"","width":"","dwt":"","flag":"","callsign":"","built_year":"","destination":"","eta":"","draft":"","wechatgroup":""},"low_confidence_fields":[]}
"""
...
def _normalize_param_group(raw: Any, keys: tuple[str, ...]) -> dict[str, str]:
...
def _normalize_update_param_groups(raw: dict[str, Any]) -> tuple[dict[str, str], dict[str, str], list[str]]:
    position_params = _normalize_param_group(raw.get("position_update_params"), POSITION_UPDATE_PARAM_KEYS)
    static_params = _normalize_param_group(raw.get("static_update_params"), STATIC_UPDATE_PARAM_KEYS)
    low_confidence = _dedupe_short_strings(raw.get("low_confidence_fields") if isinstance(raw.get("low_confidence_fields"), list) else [], 12)

    ship_type = static_params.get("ship_type", "")
    minotype = static_params.get("minotype", "")
    if ship_type and not minotype:
        static_params["minotype"] = ship_type
    elif minotype and not ship_type:
        static_params["ship_type"] = minotype
    elif ship_type and minotype and ship_type != minotype:
        marker = f"ship_type/minotype冲突：ship_type={ship_type}, minotype={minotype}"
        if marker not in low_confidence:
            low_confidence.append(marker)
        static_params["ship_type"] = ""
        static_params["minotype"] = ""

    return position_params, static_params, low_confidence


def _normalize_required_fields(raw: Any) -> list[str]:
    return _dedupe_short_strings(raw if isinstance(raw, list) else [], 20)
...
def _infer_understanding_defaults(intent: str, route: str, text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "position_update_params": _normalize_param_group({}, POSITION_UPDATE_PARAM_KEYS),
        "static_update_params": _normalize_param_group({}, STATIC_UPDATE_PARAM_KEYS),
        "low_confidence_fields": [],
...
def _normalize_customer_support_understanding_result(raw: dict[str, Any], *, text: str, intent: str, route: str) -> dict[str, Any]:
    ...
    if query_type == "authoritative_public_data":
        ...
        limit_hifleet_sites = False
    position_params, static_params, low_confidence = _normalize_update_param_groups(raw)
    ...
    return {
        ...
        "should_prefer_local_kb": prefer_local_kb,
        "should_limit_to_hifleet_sites": limit_hifleet_sites,
        "position_update_params": position_params,
        "static_update_params": static_params,
        "low_confidence_fields": low_confidence,
        "needs_clarification": bool(raw.get("needs_clarification", defaults["needs_clarification"])),
        "clarification_question": str(raw.get("clarification_question") or defaults["clarification_question"]).strip(),
        "missing_required_fields": _normalize_required_fields(raw.get("missing_required_fields")),
        "safety_flags": _normalize_required_fields(raw.get("safety_flags")),
    }
...
def _run_customer_support_intent_agent(
    *,
    ...
    return {
        ...
        "position_update_params": understanding_result["position_update_params"],
        "static_update_params": understanding_result["static_update_params"],
        "low_confidence_fields": understanding_result["low_confidence_fields"],
        "needs_clarification": understanding_result["needs_clarification"],
        "clarification_question": understanding_result["clarification_question"],
        "missing_required_fields": understanding_result["missing_required_fields"],
        "safety_flags": understanding_result["safety_flags"],
...
def _guard_customer_support_decision(
    *,
    ...
    troubleshooting_markers = ["更新慢", "更新很慢", "更新这么慢", "不更新", "不刷新", "不显示", "不准确", "延迟", "为什么", "原因", "怎么回事", "无法", "失败", "报错", "异常"]
    if agent_decision.route == "ship_update" and any(marker in q for marker in troubleshooting_markers):
        return RouteDecision("knowledge", "platform_troubleshooting", KNOWLEDGE_BUNDLE, "simple", search_depth="normal", reason="write safety guard"), "safety_guard"
    if fallback_decision.route == "ship_update" and agent_decision.route != "ship_update":
        return fallback_decision, "write_guard"
    if fallback_decision.route == "file_task" and agent_decision.route not in {"file_task", "security_refusal"}:
        return fallback_decision, "safety_guard"
    if agent_decision.route == "chart_symbol" and not attachments:
        return RouteDecision("knowledge", "platform_knowledge", KNOWLEDGE_BUNDLE, "simple", search_depth="quick", reason="chart symbol requires attachment"), "safety_guard"
    ...
    return agent_decision, "understanding_agent"
...
def _execute_customer_support_harness(
    ...
    if route == "conversation":
        ...
        trace.reasoning_trace["understanding_result"] = dict(understanding_result or {})
        answer = execute_update_chain(text, entities, tool_map, trace, understanding_result=understanding_result)
    else:
...
def _build_customer_support_agent(ctx, cfg: dict[str, Any], workspace_path: str, profile: AgentProfile, intent_hint: str = ""):
    """Build the customer_support graph led by the demand-understanding agent."""
    logger.info("[MainAgent] Building customer_support understanding-led graph")
    standard_agent = _build_standard_agent(ctx, cfg, workspace_path, profile, intent_hint=intent_hint)
    ...
    def _classify_customer_support(messages: list[AnyMessage]) -> tuple[RouteDecision, dict[str, Any], list[dict[str, Any]], dict[str, Any], dict[str, Any], str]:
        ...
        if intent_hint:
            ...
            if route_source == "understanding_agent":
    ...
    def route_node(state: CustomerSupportState) -> dict[str, Any]:
        ...
        trace.reasoning_trace = {
            ...
            "understanding_summary": {
                ...
                "needs_clarification": bool((intent_agent_result or {}).get("needs_clarification")),
                "clarification_question": str((intent_agent_result or {}).get("clarification_question", "")),
                "missing_required_fields": list((intent_agent_result or {}).get("missing_required_fields", []) or []),
                "safety_flags": list((intent_agent_result or {}).get("safety_flags", []) or []),
            },
...
def _build_lightweight_customer_support_agent(ctx, cfg: dict[str, Any], workspace_path: str, profile: AgentProfile, intent_hint: str = ""):
    ...
    def preprocess_node(state: LightweightCustomerSupportState) -> dict[str, Any]:
        ...
        if _is_ship_position_update_request(text):
            context = build_conversation_context(messages)
            raw_entities = extract_entities(text)
            understanding_result = _run_customer_support_intent_agent(
                ctx=ctx,
                cfg=cfg,
                messages=messages,
                text=text,
...
def build_agent(ctx=None, intent: str = ""):
    ...
    if profile.profile_id == "customer_support":
        ...
        logger.info("[MainAgent] Customer support understanding-led graph built successfully")
        return agent
    agent = _build_standard_agent(ctx, cfg, workspace_path, profile, intent_hint=intent_hint)
